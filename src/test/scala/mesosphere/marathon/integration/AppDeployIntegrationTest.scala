...
class AppDeployIntegrationTest extends AkkaIntegrationTest with EmbeddedMarathonTest {
  ...
  "AppDeploy" should {
    ...
    "create a simple app with a docker container and update it" in {
      ...
      updatedApp.value.app.container.flatMap(_.portMappings) should be('notEmpty)
      updatedApp.value.app.container.flatMap(_.portMappings).map(_.headOption.map(_.containerPort)) should contain(4000)
    }
  }
