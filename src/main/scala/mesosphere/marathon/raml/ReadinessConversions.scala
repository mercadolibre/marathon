...
trait ReadinessConversions {
  ...
  implicit val appReadinessProtoRamlWriter: Writes[Protos.ReadinessCheckDefinition, ReadinessCheck] = Writes { rc =>
    ReadinessCheck(
      ...
      httpStatusCodesForReady = if (rc.getHttpStatusCodeForReadyCount > 0) rc.getHttpStatusCodeForReadyList.map(_.intValue())(collection.breakOut) else ReadinessCheck.DefaultHttpStatusCodesForReady,
